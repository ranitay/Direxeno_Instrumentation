...
The experimental setup \mmd{of DIREXENO} is described in this section. This setup is 
designed to measure the spatial and temporal properties of LXe scintillation. 
\mmd{It consists of} a modular \mmd{design in order to extend its flexibility
to fulfil the requirement of any different future experiments}. 

Four main building blocks \mmd{constitutes of} the full setup, a schematics 
of which, with the interfaces \mmd{between} them is shown in Fig.~\ref{fig:fullschematics}. 
The gas handling system, \sout{which,} in normal working mode, drives the xenon from the detector 
through a purifier and \mmd{cycles} back into the detector. The cryogenic system \sout{, which} 
liquefies the xenon and delivers it to the detector system. The detector system \mmd{consists of a 
fused silica sphere that holds a small bubble of LXe target, and PMTs around the sphere. The 
supply of high volatage (HV) to the PMTs and the data readout are carried out through the 
data acquisition system (DAQ) system. These four building 
blocks are independent of each other construction wise, and can be replaced without 
altering features in the others. }
...
\mmd{As shown in Fig.~\ref{fig:fulldet}}, the full assembly is held on three separate racks, 
one for the \mmd{gas handling system, another to hold the cryogenic and the detector while 
the third one assigned for the DAQ}. The racks are joined using a 100mm bar with shock 
...
through the heat exchanger, \mmd{where the remaining gaseous} xenon is 
liquefied before it \mmd{continues} back to the chamber. A schematic of this 
...
Remote cooling is generally used in DM experiments due to background \mmd{radiati\sout{ng}on(?)} 
...
flexibility to design changes. The cryogenic system is connected to the gas \mmd{handling} system on 
one side and to the detector chamber on another side. \mmd{This sentence is not very clear: Any change in the 
system (e.g, cooler type or model) requires 
the change of that specific part without changing the detector nor the gas system.}
...
to the vacuum which prevents heat leak\mmd{age} from diffusion and convection, the entire 
IV is covered by multi layer aluminized Myler to prevent heating via radiation. \mmd{The CAD view of 
...
18i Cryogenic Temp Monitor} for temperature measurements. A \mmd{\sout{C}}cartridge-heater 
...
from the cold finger (more \mmd{\sout{dirty} impure}) allows the filling of clean LXe to 
different parts of 
the detector. 
...
The detector refers to the chamber and its inner assembly \mmd{consisting} 
...
This chamber is placed below the cryogenic system. \RanComment{We describe separately these two parts} \mmd{I feel 
that when we say ``these two parts'', it might not be a clear statement. We may either skip what we describe entirely, 
or might keep it as in the following.}
...
The detector chamber is built \mmd{in} such \mmd{a way} that apart from the interface to the cryogenic system, it can 
...
SS mini-CF flanges at the end, \mmd{which} allows \mmd{the} circulation \mmd{of xenon} through 
...
dimension of the shell. \mmd{Therefore, a sample of HPFS 8655 was tested for transmittance 
...
center of the HPFS sphere. The PMTs are held only via their voltage--divider bases, \mmd{while} 
the bases are \mmd{fixed with} M2 PEEK screws. 
