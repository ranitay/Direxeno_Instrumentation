...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\RanComment{Im missing a sentence before this explaining that the system is designed to measure spatial and temporal properties of the system}

In order to identify superradiance effects in LXe, the temporal and spatial propeties of a scintilation event should be studied and quantified. In the DIREXENO system LXe is circulated through a small spherical cavity held in a thick HPFS sphere . The sphere is surrounded (4$\pi$) by PMTs allowing a high resolution spatial and temporal measurements. As the PMTs and wires do not come in contact with the xenon, less impurities are introduced to it. The geometrical design of the system simulates a point like source of scintillation photons, so a detailed vertex reconstruction within the LXe drop is unnecessary.  (see fig xxx). 




The current system is designed with \RanComment{TODO check digitzation time} $\sim1$\,ns time resolution, $<1$\,ns synchronization between PMTs, and XX radians spatial resolution. Since the exact nature and magnitude of superradiance in LXe is yet unknown a guiding principle in the design was flexibility to upgrade or redesign any part of the system to fulfill the requirement of any future experiment. In addition we gain fast and easy recovery in case of components malfunction. This is achieved by a modular design. 

Four main building blocks constitute the setup. (i) \textbf{The gas handling system} which in normal working mode circulates the xenon through the set up and  purifies it. (ii) \textbf{The cryogenic system}, liquefies the xenon and 
...
The full assembly, shown in Fig.~\ref{fig:fulldet}, is held on three separate racks, The racks are joined using a bar with shock absorbers on both sides. 
...
During purification, LXe is taken from the detector part and
...
by a diaphragm pump(\footnote{N143 SN.12E AC 230V50HZ KNF circulation pump}) into a hot getter\footnote{MONO-TORR
PS4-MT15-R-2} which cleans the xenon from most impurities. The xenon
also passes through a mass flow controller\footnote{MKS mass flow controller} (MFC), 
this allows monitoring and controlling the amount of of heat introduced to the system. 

After the xenon is purified, it is delivered back to the cryogenic system 
through the heat exchanger, where the remaining GXe is 
...
one side and to the detector part on another side. 
The cryogenic system is built such that replacing the cryo-cooler type 
(e.g., to PTR) requires just an adaptation to the top flange.


The cryogenic system is made of two vessels, the outer vessel (OV) which holds 
the insulation vacuum, and the inner vessel (IV) which holds the xenon. In addition to the vacuum which prevents heat leakage due to diffusion and convection, the entire IV is covered by multi layer aluminized Myler to prevent heating via radiation.  
...
system, the bottom one , CF-10", is part of an IV for future detectors, and would hold xenon inside. 
